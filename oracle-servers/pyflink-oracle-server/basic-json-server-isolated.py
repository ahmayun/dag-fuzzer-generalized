#!/usr/bin/env python3
"""
PyFlink oracle server variant: user code runs in a subprocess (multiprocessing)
with join timeout, terminate/kill on overrun, and structured errors for crashes
— same isolation pattern as oracle-servers/dask-oracle-server/basic-json-server-sqlsmith.py.

On POSIX, the default process start method is fork, so the child shares the
already-initialized PyFlink JVM and TableEnvironment handles from the parent.

Oracle policy (this file only): compare optimized vs unoptimized behavior using
captured explain/plan text (stdout) and UDF-count heuristics — no materialized
output comparison (no sink / to_pandas in the worker).
"""
...
def flink_extract_error_name(e: BaseException) -> str:
    if hasattr(e, "java_exception"):
        java_error_name = e.java_exception.getClass().getName()
        try:
            return java_error_name.split(".")[-1]
        except Exception:
            return java_error_name
    return type(e).__name__


def flink_execute_in_process(
    code: str,
    result_queue: Any,
    namespace_shallow: Dict[str, Any],
    code_type: str,
    use_unopt_env: bool,
) -> None:
    """Worker: runs user code in a child; does not materialize sink for output diff."""
    ...
    try:
        ns = dict(namespace_shallow)
        ns["table_env"] = ns["unopt_table_env"] if use_unopt_env else ns["opt_table_env"]
        final_code = code
        if code_type == "sql":
            final_code = f"""print(table_env.sql_query(\"\"\"{code}\"\"\").explain())"""
        exec(final_code, ns, ns)
        result_queue.put(
            {
                "success": True,
                "error_name": "",
        ...
        result_queue.put(
            {
                "success": False,
                "error_name": flink_extract_error_name(e),
                "error_message": str(e),
                "stdout": captured_stdout.getvalue(),
                "stderr": captured_stderr.getvalue(),
                "vars": {},
...
#             self._verify_all_tables(opt_table_env, tables, catalog_name)
    ...
    def extract_error_name(self, e):
        return flink_extract_error_name(e)
    ...
    def _execute_code_with_capture(self, code, namespace, code_type):
        """Execute code with output capture and error handling (stdout/stderr from subprocess)."""
        print(f"[{datetime.now()}] Executing received code...")

        result_opt, _ns_opt = self._try_execute_code_as_is(code, namespace, code_type)
        result_unopt, _ns_unopt = self._try_execute_code_unopt(code, namespace, code_type)
    ...
    def _try_execute_code_as_is(self, code, namespace, code_type):
        """Attempt code execution and capture any errors (optimized table env)."""
        return self._try_exec_code(code, namespace, code_type, use_unopt_env=False)

    def _try_execute_code_unopt(self, code, namespace, code_type):
        """Attempt code execution and capture any errors (unoptimized table env)."""
        return self._try_exec_code(code, namespace, code_type, use_unopt_env=True)

    def _try_exec_code(self, code, namespace, code_type, use_unopt_env: bool):
        """Run user code in a child process with timeout (same pattern as Dask oracle server)."""
        result_queue = mp.Queue()
        ns_send = dict(namespace)
        p = mp.Process(
            target=flink_execute_in_process,
            args=(code, result_queue, ns_send, code_type, use_unopt_env),
        )
        p.start()
        ...
        if p.is_alive():
            ...
            return {
                ...
                "error_message": (
                    f"Code execution timed out after {GLOBAL_STATE.timeout_seconds} seconds"
                ),
                "stdout": "",
                "stderr": "",
                "vars": {},
        ...
        try:
            ...
            return {
                ...
                "error_message": "No result received from subprocess",
                "stdout": "",
                "stderr": "",
                "vars": {},
            }, namespace
    ...
    def _diff_success_results(self, result_opt, result_unopt):
        """Plan-only oracle: compare explain output (and UDF counts), not query results."""
