...
autonode_4 = autonode_6.limit(92)
autonode_5 = autonode_7.join(autonode_8, col('d_qoy_node_7') == col('c_current_cdemo_sk_node_8'))
autonode_2 = autonode_4.filter(col('d_same_day_ly_node_6') < 2)
autonode_3 = autonode_5.order_by(col('d_day_name_node_7'))
autonode_1 = autonode_2.join(autonode_3, col('d_same_day_lq_node_6') == col('c_current_addr_sk_node_8'))
sink = autonode_1.group_by(col('d_date_node_6')).select(col('d_current_month_node_6').min.alias('d_current_month_node_6'))
print(sink.explain())
...
"""
{
  ...
  "result_details": {
    "opt": {
      ...
      "error_message": "An error occurred while calling o243512082.explain.
: java.lang.RuntimeException: Error while applying rule FlinkExpandConversionRule, args [rel#492667663:AbstractConverter.BATCH_PHYSICAL.hash[0]true.[14](input=RelSubset#492667661,convention=BATCH_PHYSICAL,FlinkRelDistributionTraitDef=hash[0]true,sort=[14]), rel#492667660:BatchPhysicalLocalHashAggregate.BATCH_PHYSICAL.any.[14](input=RelSubset#492667659,groupBy=c_current_addr_sk,select=c_current_addr_sk)]
...
LogicalProject(d_current_month_node_6=[$1])
+- LogicalAggregate(group=[{2}], EXPR$0=[MIN($25)])
   +- LogicalJoin(condition=[=($22, $60)], joinType=[inner])
      :- LogicalFilter(condition=[<($21, 2)])
...
Calc(select=[EXPR$0 AS d_current_month_node_6])
+- SortAggregate(isMerge=[true], groupBy=[d_date], select=[d_date, Final_MIN(min$0) AS EXPR$0])
   +- Sort(orderBy=[d_date ASC])
      +- Exchange(distribution=[hash[d_date]])
         +- LocalSortAggregate(groupBy=[d_date], select=[d_date, Partial_MIN(d_current_month) AS min$0])
            +- Sort(orderBy=[d_date ASC])
               +- NestedLoopJoin(joinType=[InnerJoin], where=[=(d_same_day_lq, c_current_addr_sk)], select=[d_date_sk, d_date_id, d_date, d_month_seq, d_week_seq, d_quarter_seq, d_year, d_dow, d_moy, d_dom, d_qoy, d_fy_year, d_fy_quarter_seq, d_fy_week_seq, d_day_name, d_quarter_name, d_holiday, d_weekend, d_following_holiday, d_first_dom, d_last_dom, d_same_day_ly, d_same_day_lq, d_current_day, d_current_week, d_current_month, d_current_quarter, d_current_year, d_date_sk0, d_date_id0, d_date0, d_month_seq0, d_week_seq0, d_quarter_seq0, d_year0, d_dow0, d_moy0, d_dom0, d_qoy0, d_fy_year0, d_fy_quarter_seq0, d_fy_week_seq0, d_day_name0, d_quarter_name0, d_holiday0, d_weekend0, d_following_holiday0, d_first_dom0, d_last_dom0, d_same_day_ly0, d_same_day_lq0, d_current_day0, d_current_week0, d_current_month0, d_current_quarter0, d_current_year0, c_customer_sk, c_customer_id, c_current_cdemo_sk, c_current_hdemo_sk, c_current_addr_sk, c_first_shipto_date_sk, c_first_sales_date_sk, c_salutation, c_first_name, c_last_name, c_preferred_cust_flag, c_birth_day, c_birth_month, c_birth_year, c_birth_country, c_login, c_email_address, c_last_review_date], build=[right])
                  :- Calc(select=[d_date_sk, d_date_id, d_date, d_month_seq, d_week_seq, d_quarter_seq, d_year, d_dow, d_moy, d_dom, d_qoy, d_fy_year, d_fy_quarter_seq, d_fy_week_seq, d_day_name, d_quarter_name, d_holiday, d_weekend, d_following_holiday, d_first_dom, d_last_dom, d_same_day_ly, d_same_day_lq, d_current_day, d_current_week, d_current_month, d_current_quarter, d_current_year], where=[<(d_same_day_ly, 2)])
                  :  +- Limit(offset=[0], fetch=[92], global=[true])
                  :     +- Exchange(distribution=[single])
                  :        +- Limit(offset=[0], fetch=[92], global=[false])
                  :           +- TableSourceScan(table=[[default_catalog, default_database, date_dim, limit=[92]]], fields=[d_date_sk, d_date_id, d_date, d_month_seq, d_week_seq, d_quarter_seq, d_year, d_dow, d_moy, d_dom, d_qoy, d_fy_year, d_fy_quarter_seq, d_fy_week_seq, d_day_name, d_quarter_name, d_holiday, d_weekend, d_following_holiday, d_first_dom, d_last_dom, d_same_day_ly, d_same_day_lq, d_current_day, d_current_week, d_current_month, d_current_quarter, d_current_year])
                  +- Exchange(distribution=[broadcast])
                     +- SortLimit(orderBy=[d_day_name ASC], offset=[0], fetch=[1], global=[true])
                        +- Exchange(distribution=[single])
                           +- SortLimit(orderBy=[d_day_name ASC], offset=[0], fetch=[1], global=[false])
                              +- HashJoin(joinType=[InnerJoin], where=[=(d_qoy, c_current_cdemo_sk)], select=[d_date_sk, d_date_id, d_date, d_month_seq, d_week_seq, d_quarter_seq, d_year, d_dow, d_moy, d_dom, d_qoy, d_fy_year, d_fy_quarter_seq, d_fy_week_seq, d_day_name, d_quarter_name, d_holiday, d_weekend, d_following_holiday, d_first_dom, d_last_dom, d_same_day_ly, d_same_day_lq, d_current_day, d_current_week, d_current_month, d_current_quarter, d_current_year, c_customer_sk, c_customer_id, c_current_cdemo_sk, c_current_hdemo_sk, c_current_addr_sk, c_first_shipto_date_sk, c_first_sales_date_sk, c_salutation, c_first_name, c_last_name, c_preferred_cust_flag, c_birth_day, c_birth_month, c_birth_year, c_birth_country, c_login, c_email_address, c_last_review_date], build=[right])
                                 :- Exchange(distribution=[hash[d_qoy]])
