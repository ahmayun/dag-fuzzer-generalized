...
autonode_6 = autonode_8.order_by(col('cc_sq_ft_node_8'))
autonode_5 = autonode_7.order_by(col('cd_marital_status_node_7'))
autonode_4 = autonode_6.order_by(col('cc_street_name_node_8'))
autonode_3 = autonode_5.alias('Htpm7')
autonode_2 = autonode_3.join(autonode_4, col('cd_credit_rating_node_7') == col('cc_zip_node_8'))
autonode_1 = autonode_2.group_by(col('cc_company_name_node_8')).select(col('cc_gmt_offset_node_8').min.alias('cc_gmt_offset_node_8'))
sink = autonode_1.group_by(col('cc_gmt_offset_node_8')).select(call('preloaded_udf_agg', col('cc_gmt_offset_node_8')).alias('cc_gmt_offset_node_8'))
print(sink.explain())
...
"""
{
  ...
  "result_details": {
    "opt": {
      ...
      "error_message": "An error occurred while calling o318060698.explain.
: java.lang.RuntimeException: Error while applying rule FlinkExpandConversionRule, args [rel#641163658:AbstractConverter.BATCH_PHYSICAL.hash[0]true.[2](input=RelSubset#641163656,convention=BATCH_PHYSICAL,FlinkRelDistributionTraitDef=hash[0]true,sort=[2]), rel#641163655:BatchPhysicalLocalHashAggregate.BATCH_PHYSICAL.any.[2](input=RelSubset#641163654,groupBy=cd_credit_rating,select=cd_credit_rating)]
...
LogicalProject(cc_gmt_offset_node_8=[$1])
+- LogicalAggregate(group=[{0}], EXPR$0=[preloaded_udf_agg($0)])
   +- LogicalProject(cc_gmt_offset_node_8=[$1])
      +- LogicalAggregate(group=[{28}], EXPR$0=[MIN($38)])
         +- LogicalJoin(condition=[=($5, $36)], joinType=[inner])
            :- LogicalProject(Htpm7=[AS($0, _UTF-16LE'Htpm7')], cd_gender_node_7=[$1], cd_marital_status_node_7=[$2], cd_education_status_node_7=[$3], cd_purchase_estimate_node_7=[$4], cd_credit_rating_node_7=[$5], cd_dep_count_node_7=[$6], cd_dep_employed_count_node_7=[$7], cd_dep_college_count_node_7=[$8])
...
Calc(select=[EXPR$0 AS cc_gmt_offset_node_8])
+- PythonGroupAggregate(groupBy=[cc_gmt_offset_node_8], select=[cc_gmt_offset_node_8, preloaded_udf_agg(cc_gmt_offset_node_8) AS EXPR$0])
   +- Sort(orderBy=[cc_gmt_offset_node_8 ASC])
      +- Exchange(distribution=[hash[cc_gmt_offset_node_8]])
         +- Calc(select=[EXPR$0 AS cc_gmt_offset_node_8])
            +- SortAggregate(isMerge=[false], groupBy=[cc_company_name], select=[cc_company_name, MIN(cc_gmt_offset) AS EXPR$0])
               +- Sort(orderBy=[cc_company_name ASC])
                  +- NestedLoopJoin(joinType=[InnerJoin], where=[=(cd_credit_rating_node_7, cc_zip)], select=[Htpm7, cd_gender_node_7, cd_marital_status_node_7, cd_education_status_node_7, cd_purchase_estimate_node_7, cd_credit_rating_node_7, cd_dep_count_node_7, cd_dep_employed_count_node_7, cd_dep_college_count_node_7, cc_call_center_sk, cc_call_center_id, cc_rec_start_date, cc_rec_end_date, cc_closed_date_sk, cc_open_date_sk, cc_name, cc_class, cc_employees, cc_sq_ft, cc_hours, cc_manager, cc_mkt_id, cc_mkt_class, cc_mkt_desc, cc_market_manager, cc_division, cc_division_name, cc_company, cc_company_name, cc_street_number, cc_street_name, cc_street_type, cc_suite_number, cc_city, cc_county, cc_state, cc_zip, cc_country, cc_gmt_offset, cc_tax_percentage], build=[left])
                     :- Exchange(distribution=[broadcast])
                     :  +- Calc(select=[cd_demo_sk AS Htpm7, cd_gender AS cd_gender_node_7, cd_marital_status AS cd_marital_status_node_7, cd_education_status AS cd_education_status_node_7, cd_purchase_estimate AS cd_purchase_estimate_node_7, cd_credit_rating AS cd_credit_rating_node_7, cd_dep_count AS cd_dep_count_node_7, cd_dep_employed_count AS cd_dep_employed_count_node_7, cd_dep_college_count AS cd_dep_college_count_node_7])
                     ...
                     +- Exchange(distribution=[hash[cc_company_name]])
                        +- SortLimit(orderBy=[cc_street_name ASC], offset=[0], fetch=[1], global=[true])
                           +- Exchange(distribution=[single])
                              +- SortLimit(orderBy=[cc_street_name ASC], offset=[0], fetch=[1], global=[false])
                                 +- SortLimit(orderBy=[cc_sq_ft ASC], offset=[0], fetch=[1], global=[true])
                                    +- Exchange(distribution=[single])
                                       +- SortLimit(orderBy=[cc_sq_ft ASC], offset=[0], fetch=[1], global=[false])
...
Calc(select=[EXPR$0 AS cc_gmt_offset_node_8])
+- PythonGroupAggregate(groupBy=[cc_gmt_offset_node_8], select=[cc_gmt_offset_node_8, preloaded_udf_agg(cc_gmt_offset_node_8) AS EXPR$0])
   +- Exchange(distribution=[keep_input_as_is[hash[cc_gmt_offset_node_8]]])
      +- Sort(orderBy=[cc_gmt_offset_node_8 ASC])
         +- Exchange(distribution=[hash[cc_gmt_offset_node_8]])
            +- Calc(select=[EXPR$0 AS cc_gmt_offset_node_8])
               +- SortAggregate(isMerge=[false], groupBy=[cc_company_name], select=[cc_company_name, MIN(cc_gmt_offset) AS EXPR$0])
                  +- Exchange(distribution=[forward])
                     +- Sort(orderBy=[cc_company_name ASC])
                        +- Exchange(distribution=[keep_input_as_is[hash[cc_company_name]]])
                           +- NestedLoopJoin(joinType=[InnerJoin], where=[(cd_credit_rating_node_7 = cc_zip)], select=[Htpm7, cd_gender_node_7, cd_marital_status_node_7, cd_education_status_node_7, cd_purchase_estimate_node_7, cd_credit_rating_node_7, cd_dep_count_node_7, cd_dep_employed_count_node_7, cd_dep_college_count_node_7, cc_call_center_sk, cc_call_center_id, cc_rec_start_date, cc_rec_end_date, cc_closed_date_sk, cc_open_date_sk, cc_name, cc_class, cc_employees, cc_sq_ft, cc_hours, cc_manager, cc_mkt_id, cc_mkt_class, cc_mkt_desc, cc_market_manager, cc_division, cc_division_name, cc_company, cc_company_name, cc_street_number, cc_street_name, cc_street_type, cc_suite_number, cc_city, cc_county, cc_state, cc_zip, cc_country, cc_gmt_offset, cc_tax_percentage], build=[left])
                              :- Exchange(distribution=[broadcast])
                              :  +- Calc(select=[cd_demo_sk AS Htpm7, cd_gender AS cd_gender_node_7, cd_marital_status AS cd_marital_status_node_7, cd_education_status AS cd_education_status_node_7, cd_purchase_estimate AS cd_purchase_estimate_node_7, cd_credit_rating AS cd_credit_rating_node_7, cd_dep_count AS cd_dep_count_node_7, cd_dep_employed_count AS cd_dep_employed_count_node_7, cd_dep_college_count AS cd_dep_college_count_node_7])
                              :     +- SortLimit(orderBy=[cd_marital_status ASC], offset=[0], fetch=[1], global=[true])
