...
autonode_6 = autonode_8.join(autonode_9, col('ss_sold_date_sk_node_8') == col('wp_link_count_node_9'))
autonode_5 = autonode_7.select(col('cd_purchase_estimate_node_7'))
autonode_4 = autonode_6.order_by(col('wp_access_date_sk_node_9'))
autonode_3 = autonode_5.distinct()
autonode_2 = autonode_3.join(autonode_4, col('ss_cdemo_sk_node_8') == col('cd_purchase_estimate_node_7'))
autonode_1 = autonode_2.group_by(col('ss_item_sk_node_8')).select(col('ss_store_sk_node_8').max.alias('ss_store_sk_node_8'))
sink = autonode_1.filter(col('ss_store_sk_node_8') > -13)
print(sink.explain())
...
LogicalFilter(condition=[>($0, -13)])
+- LogicalProject(ss_store_sk_node_8=[$1])
   +- LogicalAggregate(group=[{3}], EXPR$0=[MAX($8)])
      +- LogicalJoin(condition=[=($5, $0)], joinType=[inner])
         :- LogicalAggregate(group=[{0}])
         :  +- LogicalProject(cd_purchase_estimate_node_7=[$4])
...
Calc(select=[EXPR$0], where=[>(EXPR$0, -13)])
+- HashAggregate(isMerge=[true], groupBy=[ss_item_sk], select=[ss_item_sk, Final_MAX(max$0) AS EXPR$0])
   +- Exchange(distribution=[hash[ss_item_sk]])
      +- LocalHashAggregate(groupBy=[ss_item_sk], select=[ss_item_sk, Partial_MAX(ss_store_sk) AS max$0])
         +- NestedLoopJoin(joinType=[InnerJoin], where=[=(ss_cdemo_sk, cd_purchase_estimate)], select=[cd_purchase_estimate, ss_sold_date_sk, ss_sold_time_sk, ss_item_sk, ss_customer_sk, ss_cdemo_sk, ss_hdemo_sk, ss_addr_sk, ss_store_sk, ss_promo_sk, ss_ticket_number, ss_quantity, ss_wholesale_cost, ss_list_price, ss_sales_price, ss_ext_discount_amt, ss_ext_sales_price, ss_ext_wholesale_cost, ss_ext_list_price, ss_ext_tax, ss_coupon_amt, ss_net_paid, ss_net_paid_inc_tax, ss_net_profit, wp_web_page_sk, wp_web_page_id, wp_rec_start_date, wp_rec_end_date, wp_creation_date_sk, wp_access_date_sk, wp_autogen_flag, wp_customer_sk, wp_url, wp_type, wp_char_count, wp_link_count, wp_image_count, wp_max_ad_count], build=[left])
            :- Exchange(distribution=[broadcast])
            :  +- HashAggregate(isMerge=[true], groupBy=[cd_purchase_estimate], select=[cd_purchase_estimate])
            :     +- Exchange(distribution=[hash[cd_purchase_estimate]])
            :        +- LocalHashAggregate(groupBy=[cd_purchase_estimate], select=[cd_purchase_estimate])
            :           +- TableSourceScan(table=[[default_catalog, default_database, customer_demographics, project=[cd_purchase_estimate], metadata=[]]], fields=[cd_purchase_estimate])
            +- Sort(orderBy=[wp_access_date_sk ASC])
               +- Exchange(distribution=[single])
                  +- HashJoin(joinType=[InnerJoin], where=[=(ss_sold_date_sk, wp_link_count)], select=[ss_sold_date_sk, ss_sold_time_sk, ss_item_sk, ss_customer_sk, ss_cdemo_sk, ss_hdemo_sk, ss_addr_sk, ss_store_sk, ss_promo_sk, ss_ticket_number, ss_quantity, ss_wholesale_cost, ss_list_price, ss_sales_price, ss_ext_discount_amt, ss_ext_sales_price, ss_ext_wholesale_cost, ss_ext_list_price, ss_ext_tax, ss_coupon_amt, ss_net_paid, ss_net_paid_inc_tax, ss_net_profit, wp_web_page_sk, wp_web_page_id, wp_rec_start_date, wp_rec_end_date, wp_creation_date_sk, wp_access_date_sk, wp_autogen_flag, wp_customer_sk, wp_url, wp_type, wp_char_count, wp_link_count, wp_image_count, wp_max_ad_count], isBroadcast=[true], build=[right])
...
",
    ...
    "unopt": {
      ...
      "error_message": "An error occurred while calling o45195640.explain.
: java.lang.RuntimeException: Error while applying rule FlinkExpandConversionRule, args [rel#89725252:AbstractConverter.BATCH_PHYSICAL.hash[0, 1]true.[28](input=RelSubset#89725250,convention=BATCH_PHYSICAL,FlinkRelDistributionTraitDef=hash[0, 1]true,sort=[28]), rel#89725249:BatchPhysicalLocalHashAggregate.BATCH_PHYSICAL.any.[28](input=RelSubset#89725248,groupBy=ss_item_sk, ss_cdemo_sk,select=ss_item_sk, ss_cdemo_sk, Partial_MAX(ss_store_sk) AS max$0)]
