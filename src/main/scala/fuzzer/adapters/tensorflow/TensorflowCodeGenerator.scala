package fuzzer.adapters.tensorflow
...
class TensorflowCodeGenerator(config: FuzzerConfig, spec: JsValue, dag2CodeFunc: Graph[DFOperator] => SourceCode) extends CodeGenerator {

  override def getDag2CodeFunc: Graph[DFOperator] => SourceCode = dag2CodeFunc
}

class TensorflowDataAdapter(config: FuzzerConfig) extends DataAdapter {
  ...
  override def loadData(executor: CodeExecutor, filterF: String => Boolean = _ => true): Unit = {
    ...
    success match {
      case Some(true) =>
      case _ => throw new Exception("Tensorflow server failed to load data!")
    }
  }
...
class TensorflowCodeExecutor(config: FuzzerConfig, spec: JsValue) extends CodeExecutor {
