...
autonode_3 = autonode_5.order_by(col('cp_start_date_sk_node_5'))
autonode_2 = autonode_3.join(autonode_4, col('sr_store_sk_node_6') == col('cp_catalog_page_number_node_5'))
autonode_1 = autonode_2.filter(col('sr_store_credit_node_6') > -31.834644079208374)
sink = autonode_1.group_by(col('sr_addr_sk_node_6')).select(col('sr_store_credit_node_6').max.alias('sr_store_credit_node_6'))
print(sink.explain())
...
"""
{
  ...
  "result_details": {
    "opt": {
      ...
      "error_message": "An error occurred while calling o72768769.explain.
: java.lang.RuntimeException: Error while applying rule FlinkExpandConversionRule, args [rel#146090731:AbstractConverter.BATCH_PHYSICAL.hash[0]true.[2](input=RelSubset#146090729,convention=BATCH_PHYSICAL,FlinkRelDistributionTraitDef=hash[0]true,sort=[2]), rel#146090728:BatchPhysicalLocalHashAggregate.BATCH_PHYSICAL.any.[2](input=RelSubset#146090727,groupBy=cp_catalog_page_number,select=cp_catalog_page_number)]
...
LogicalProject(sr_store_credit_node_6=[$1])
+- LogicalAggregate(group=[{15}], EXPR$0=[MAX($27)])
   +- LogicalFilter(condition=[>($27, -3.1834644079208374E1:DOUBLE)])
      +- LogicalJoin(condition=[=($16, $6)], joinType=[inner])
...
Calc(select=[EXPR$0 AS sr_store_credit_node_6])
+- SortAggregate(isMerge=[true], groupBy=[sr_addr_sk], select=[sr_addr_sk, Final_MAX(max$0) AS EXPR$0])
   +- Sort(orderBy=[sr_addr_sk ASC])
      +- Exchange(distribution=[hash[sr_addr_sk]])
         +- LocalSortAggregate(groupBy=[sr_addr_sk], select=[sr_addr_sk, Partial_MAX(sr_store_credit) AS max$0])
            +- Sort(orderBy=[sr_addr_sk ASC])
               +- NestedLoopJoin(joinType=[InnerJoin], where=[=(sr_store_sk, cp_catalog_page_number)], select=[cp_catalog_page_sk, cp_catalog_page_id, cp_start_date_sk, cp_end_date_sk, cp_department, cp_catalog_number, cp_catalog_page_number, cp_description, cp_type, sr_returned_date_sk, sr_return_time_sk, sr_item_sk, sr_customer_sk, sr_cdemo_sk, sr_hdemo_sk, sr_addr_sk, sr_store_sk, sr_reason_sk, sr_ticket_number, sr_return_quantity, sr_return_amt, sr_return_tax, sr_return_amt_inc_tax, sr_fee, sr_return_ship_cost, sr_refunded_cash, sr_reversed_charge, sr_store_credit, sr_net_loss], build=[left])
                  :- Exchange(distribution=[broadcast])
                  :  +- SortLimit(orderBy=[cp_start_date_sk ASC], offset=[0], fetch=[1], global=[true])
                  :     +- Exchange(distribution=[single])
                  :        +- SortLimit(orderBy=[cp_start_date_sk ASC], offset=[0], fetch=[1], global=[false])
                  ...
                  +- Calc(select=[sr_returned_date_sk, sr_return_time_sk, sr_item_sk, sr_customer_sk, sr_cdemo_sk, sr_hdemo_sk, sr_addr_sk, sr_store_sk, sr_reason_sk, sr_ticket_number, sr_return_quantity, sr_return_amt, sr_return_tax, sr_return_amt_inc_tax, sr_fee, sr_return_ship_cost, sr_refunded_cash, sr_reversed_charge, sr_store_credit, sr_net_loss], where=[>(sr_store_credit, -3.1834644079208374E1)])
                     +- Limit(offset=[0], fetch=[34], global=[true])
                        +- Exchange(distribution=[single])
                           +- Limit(offset=[0], fetch=[34], global=[false])
                              +- TableSourceScan(table=[[default_catalog, default_database, store_returns, limit=[34]]], fields=[sr_returned_date_sk, sr_return_time_sk, sr_item_sk, sr_customer_sk, sr_cdemo_sk, sr_hdemo_sk, sr_addr_sk, sr_store_sk, sr_reason_sk, sr_ticket_number, sr_return_quantity, sr_return_amt, sr_return_tax, sr_return_amt_inc_tax, sr_fee, sr_return_ship_cost, sr_refunded_cash, sr_reversed_charge, sr_store_credit, sr_net_loss])
