...
autonode_7 = table_env.from_path("ship_mode").select(*[col(column_name).alias(f"{column_name}_node_7") for column_name in table_env.from_path("ship_mode").get_schema().get_field_names()])
autonode_4 = autonode_6.order_by(col('cp_start_date_sk_node_6'))
autonode_5 = autonode_7.order_by(col('sm_carrier_node_7'))
autonode_3 = autonode_4.join(autonode_5, col('cp_end_date_sk_node_6') == col('sm_ship_mode_sk_node_7'))
autonode_2 = autonode_3.group_by(col('cp_end_date_sk_node_6')).select(col('sm_carrier_node_7').min.alias('sm_carrier_node_7'))
autonode_1 = autonode_2.limit(65)
sink = autonode_1.select(col('sm_carrier_node_7'))
print(sink.explain())
...
"""
{
  ...
  "result_details": {
    "opt": {
      ...
      "error_message": "An error occurred while calling o292115328.explain.
: java.lang.RuntimeException: Error while applying rule FlinkExpandConversionRule, args [rel#589808646:AbstractConverter.BATCH_PHYSICAL.hash[0]true.[2](input=RelSubset#589808644,convention=BATCH_PHYSICAL,FlinkRelDistributionTraitDef=hash[0]true,sort=[2]), rel#589808643:BatchPhysicalLocalHashAggregate.BATCH_PHYSICAL.any.[2](input=RelSubset#589808642,groupBy=cp_end_date_sk,select=cp_end_date_sk)]
...
LogicalProject(sm_carrier_node_7=[$0])
+- LogicalSort(fetch=[65])
   +- LogicalProject(sm_carrier_node_7=[$1])
      +- LogicalAggregate(group=[{3}], EXPR$0=[MIN($13)])
         +- LogicalJoin(condition=[=($3, $9)], joinType=[inner])
            ...
            +- LogicalSort(sort0=[$4], dir0=[ASC])
               +- LogicalProject(sm_ship_mode_sk_node_7=[$0], sm_ship_mode_id_node_7=[$1], sm_type_node_7=[$2], sm_code_node_7=[$3], sm_carrier_node_7=[$4], sm_contract_node_7=[$5])
...
+- Exchange(distribution=[single])
   +- Limit(offset=[0], fetch=[65], global=[false])
      +- Calc(select=[EXPR$0 AS sm_carrier_node_7])
         +- SortAggregate(isMerge=[false], groupBy=[cp_end_date_sk], select=[cp_end_date_sk, MIN(sm_carrier) AS EXPR$0])
            +- Sort(orderBy=[cp_end_date_sk ASC])
               +- NestedLoopJoin(joinType=[InnerJoin], where=[=(cp_end_date_sk, sm_ship_mode_sk)], select=[cp_catalog_page_sk, cp_catalog_page_id, cp_start_date_sk, cp_end_date_sk, cp_department, cp_catalog_number, cp_catalog_page_number, cp_description, cp_type, sm_ship_mode_sk, sm_ship_mode_id, sm_type, sm_code, sm_carrier, sm_contract], build=[right])
                  :- Exchange(distribution=[hash[cp_end_date_sk]])
                  ...
                  +- Exchange(distribution=[broadcast])
                     +- SortLimit(orderBy=[sm_carrier ASC], offset=[0], fetch=[1], global=[true])
                        +- Exchange(distribution=[single])
                           +- SortLimit(orderBy=[sm_carrier ASC], offset=[0], fetch=[1], global=[false])
      ...
      +- Calc(select=[EXPR$0 AS sm_carrier_node_7])
         +- SortAggregate(isMerge=[false], groupBy=[cp_end_date_sk], select=[cp_end_date_sk, MIN(sm_carrier) AS EXPR$0])
            +- Exchange(distribution=[forward])
               +- Sort(orderBy=[cp_end_date_sk ASC])
                  +- Exchange(distribution=[keep_input_as_is[hash[cp_end_date_sk]]])
                     +- NestedLoopJoin(joinType=[InnerJoin], where=[(cp_end_date_sk = sm_ship_mode_sk)], select=[cp_catalog_page_sk, cp_catalog_page_id, cp_start_date_sk, cp_end_date_sk, cp_department, cp_catalog_number, cp_catalog_page_number, cp_description, cp_type, sm_ship_mode_sk, sm_ship_mode_id, sm_type, sm_code, sm_carrier, sm_contract], build=[right])
                        :- Exchange(distribution=[hash[cp_end_date_sk]])
                        :  +- SortLimit(orderBy=[cp_start_date_sk ASC], offset=[0], fetch=[1], global=[true])
                        :     +- Exchange(distribution=[single])
