...
autonode_4 = autonode_6.order_by(col('i_units_node_6'))
autonode_5 = autonode_7.limit(19)
autonode_2 = autonode_4.order_by(col('i_item_desc_node_6'))
autonode_3 = autonode_5.add_columns(lit("hello"))
autonode_1 = autonode_2.join(autonode_3, col('i_item_sk_node_6') == col('s_market_id_node_7'))
sink = autonode_1.group_by(col('s_company_id_node_7')).select(col('s_state_node_7').min.alias('s_state_node_7'))
print(sink.explain())
...
"""
{
  ...
  "result_details": {
    "opt": {
      ...
      "error_message": "An error occurred while calling o220211253.explain.
: java.lang.RuntimeException: Error while applying rule FlinkExpandConversionRule, args [rel#445836400:AbstractConverter.BATCH_PHYSICAL.hash[0]true.[4](input=RelSubset#445836398,convention=BATCH_PHYSICAL,FlinkRelDistributionTraitDef=hash[0]true,sort=[4]), rel#445836397:BatchPhysicalLocalHashAggregate.BATCH_PHYSICAL.any.[4](input=RelSubset#445836396,groupBy=i_item_sk,select=i_item_sk)]
...
LogicalProject(s_state_node_7=[$1])
+- LogicalAggregate(group=[{38}], EXPR$0=[MIN($46)])
...
Calc(select=[EXPR$0 AS s_state_node_7])
+- SortAggregate(isMerge=[false], groupBy=[s_company_id_node_7], select=[s_company_id_node_7, MIN(s_state_node_7) AS EXPR$0])
   +- Sort(orderBy=[s_company_id_node_7 ASC])
      +- Exchange(distribution=[hash[s_company_id_node_7]])
         +- NestedLoopJoin(joinType=[InnerJoin], where=[=(i_item_sk, s_market_id_node_7)], select=[i_item_sk, i_item_id, i_rec_start_date, i_rec_end_date, i_item_desc, i_current_price, i_wholesale_cost, i_brand_id, i_brand, i_class_id, i_class, i_category_id, i_category, i_manufact_id, i_manufact, i_size, i_formulation, i_color, i_units, i_container, i_manager_id, i_product_name, s_store_sk_node_7, s_store_id_node_7, s_rec_start_date_node_7, s_rec_end_date_node_7, s_closed_date_sk_node_7, s_store_name_node_7, s_number_employees_node_7, s_floor_space_node_7, s_hours_node_7, s_manager_node_7, s_market_id_node_7, s_geography_class_node_7, s_market_desc_node_7, s_market_manager_node_7, s_division_id_node_7, s_division_name_node_7, s_company_id_node_7, s_company_name_node_7, s_street_number_node_7, s_street_name_node_7, s_street_type_node_7, s_suite_number_node_7, s_city_node_7, s_county_node_7, s_state_node_7, s_zip_node_7, s_country_node_7, s_gmt_offset_node_7, s_tax_precentage_node_7, _c29], build=[left])
            :- Exchange(distribution=[broadcast])
            :  +- SortLimit(orderBy=[i_item_desc ASC], offset=[0], fetch=[1], global=[true])
            :     +- Exchange(distribution=[single])
            :        +- SortLimit(orderBy=[i_item_desc ASC], offset=[0], fetch=[1], global=[false])
