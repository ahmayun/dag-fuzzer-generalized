...
autonode_4 = autonode_6.order_by(col('cs_list_price_node_6'))
autonode_5 = autonode_7.alias('hYH1U')
autonode_2 = autonode_4.limit(46)
autonode_3 = autonode_5.order_by(col('inv_quantity_on_hand_node_7'))
autonode_1 = autonode_2.join(autonode_3, col('cs_item_sk_node_6') == col('inv_item_sk_node_7'))
sink = autonode_1.group_by(col('cs_item_sk_node_6')).select(col('cs_ext_ship_cost_node_6').max.alias('cs_ext_ship_cost_node_6'))
print(sink.explain())
...
"""
{
  ...
  "result_details": {
    "opt": {
      ...
      "error_message": "An error occurred while calling o69469023.explain.
: java.lang.RuntimeException: Error while applying rule FlinkExpandConversionRule, args [rel#139078786:AbstractConverter.BATCH_PHYSICAL.hash[0]true.[20](input=RelSubset#139078784,convention=BATCH_PHYSICAL,FlinkRelDistributionTraitDef=hash[0]true,sort=[20]), rel#139078783:BatchPhysicalLocalHashAggregate.BATCH_PHYSICAL.any.[20](input=RelSubset#139078782,groupBy=cs_item_sk,select=cs_item_sk, Partial_MAX(cs_ext_ship_cost) AS max$0)]
...
+- LogicalAggregate(group=[{15}], EXPR$0=[MAX($28)])
   +- LogicalJoin(condition=[=($15, $35)], joinType=[inner])
      :- LogicalSort(sort0=[$20], dir0=[ASC], fetch=[46])
      ...
      +- LogicalSort(sort0=[$3], dir0=[ASC])
         +- LogicalProject(hYH1U=[AS($0, _UTF-16LE'hYH1U')], inv_item_sk_node_7=[$1], inv_warehouse_sk_node_7=[$2], inv_quantity_on_hand_node_7=[$3])
            +- LogicalTableScan(table=[[default_catalog, default_database, inventory]])

== Optimized Physical Plan ==
Calc(select=[EXPR$0 AS cs_ext_ship_cost_node_6])
+- SortAggregate(isMerge=[true], groupBy=[cs_item_sk], select=[cs_item_sk, Final_MAX(max$0) AS EXPR$0])
   +- Sort(orderBy=[cs_item_sk ASC])
      +- Exchange(distribution=[hash[cs_item_sk]])
         +- LocalSortAggregate(groupBy=[cs_item_sk], select=[cs_item_sk, Partial_MAX(cs_ext_ship_cost) AS max$0])
            +- Sort(orderBy=[cs_item_sk ASC])
               +- NestedLoopJoin(joinType=[InnerJoin], where=[=(cs_item_sk, inv_item_sk_node_7)], select=[cs_sold_date_sk, cs_sold_time_sk, cs_ship_date_sk, cs_bill_customer_sk, cs_bill_cdemo_sk, cs_bill_hdemo_sk, cs_bill_addr_sk, cs_ship_customer_sk, cs_ship_cdemo_sk, cs_ship_hdemo_sk, cs_ship_addr_sk, cs_call_center_sk, cs_catalog_page_sk, cs_ship_mode_sk, cs_warehouse_sk, cs_item_sk, cs_promo_sk, cs_order_number, cs_quantity, cs_wholesale_cost, cs_list_price, cs_sales_price, cs_ext_discount_amt, cs_ext_sales_price, cs_ext_wholesale_cost, cs_ext_list_price, cs_ext_tax, cs_coupon_amt, cs_ext_ship_cost, cs_net_paid, cs_net_paid_inc_tax, cs_net_paid_inc_ship, cs_net_paid_inc_ship_tax, cs_net_profit, hYH1U, inv_item_sk_node_7, inv_warehouse_sk_node_7, inv_quantity_on_hand_node_7], build=[right])
                  :- SortLimit(orderBy=[cs_list_price ASC], offset=[0], fetch=[46], global=[true])
                  :  +- Exchange(distribution=[single])
                  :     +- SortLimit(orderBy=[cs_list_price ASC], offset=[0], fetch=[46], global=[false])
                  ...
                  +- Exchange(distribution=[broadcast])
                     +- Calc(select=[inv_date_sk AS hYH1U, inv_item_sk AS inv_item_sk_node_7, inv_warehouse_sk AS inv_warehouse_sk_node_7, inv_quantity_on_hand AS inv_quantity_on_hand_node_7])
