...
autonode_4 = autonode_6.order_by(col('ss_net_paid_node_6'))
autonode_5 = autonode_7.filter(col('t_minute_node_7') < 40)
autonode_2 = autonode_4.add_columns(lit("hello"))
autonode_3 = autonode_5.group_by(col('t_shift_node_7')).select(col('t_hour_node_7').min.alias('t_hour_node_7'))
autonode_1 = autonode_2.join(autonode_3, col('ss_ticket_number_node_6') == col('t_hour_node_7'))
sink = autonode_1.group_by(col('ss_net_paid_inc_tax_node_6')).select(col('ss_promo_sk_node_6').max.alias('ss_promo_sk_node_6'))
print(sink.explain())
...
+- LogicalAggregate(group=[{21}], EXPR$0=[MAX($8)])
   +- LogicalJoin(condition=[=($9, $24)], joinType=[inner])
      ...
      +- LogicalProject(t_hour_node_7=[$1])
         +- LogicalAggregate(group=[{7}], EXPR$0=[MIN($3)])
            +- LogicalFilter(condition=[<($4, 40)])
         ...
         +- HashJoin(joinType=[InnerJoin], where=[=(ss_ticket_number_node_6, t_hour_node_7)], select=[ss_sold_date_sk_node_6, ss_sold_time_sk_node_6, ss_item_sk_node_6, ss_customer_sk_node_6, ss_cdemo_sk_node_6, ss_hdemo_sk_node_6, ss_addr_sk_node_6, ss_store_sk_node_6, ss_promo_sk_node_6, ss_ticket_number_node_6, ss_quantity_node_6, ss_wholesale_cost_node_6, ss_list_price_node_6, ss_sales_price_node_6, ss_ext_discount_amt_node_6, ss_ext_sales_price_node_6, ss_ext_wholesale_cost_node_6, ss_ext_list_price_node_6, ss_ext_tax_node_6, ss_coupon_amt_node_6, ss_net_paid_node_6, ss_net_paid_inc_tax_node_6, ss_net_profit_node_6, _c23, t_hour_node_7], isBroadcast=[true], build=[right])
            ...
            +- Exchange(distribution=[broadcast])
               +- Calc(select=[EXPR$0 AS t_hour_node_7])
                  +- HashAggregate(isMerge=[true], groupBy=[t_shift], select=[t_shift, Final_MIN(min$0) AS EXPR$0])
                     +- Exchange(distribution=[hash[t_shift]])
                        +- LocalHashAggregate(groupBy=[t_shift], select=[t_shift, Partial_MIN(t_hour) AS min$0])
                           +- Calc(select=[t_time_sk, t_time_id, t_time, t_hour, t_minute, t_second, t_am_pm, t_shift, t_sub_shift, t_meal_time], where=[<(t_minute, 40)])
                              +- TableSourceScan(table=[[default_catalog, default_database, time_dim, filter=[<(t_minute, 40)]]], fields=[t_time_sk, t_time_id, t_time, t_hour, t_minute, t_second, t_am_pm, t_shift, t_sub_shift, t_meal_time])
...
",
    ...
    "unopt": {
      ...
      "error_message": "An error occurred while calling o262219626.explain.
: java.lang.RuntimeException: Error while applying rule FlinkExpandConversionRule, args [rel#530044142:AbstractConverter.BATCH_PHYSICAL.hash[0, 1]true.[20](input=RelSubset#530044140,convention=BATCH_PHYSICAL,FlinkRelDistributionTraitDef=hash[0, 1]true,sort=[20]), rel#530044139:BatchPhysicalLocalHashAggregate.BATCH_PHYSICAL.any.[20](input=RelSubset#530044138,groupBy=ss_ticket_number, ss_net_paid_inc_tax,select=ss_ticket_number, ss_net_paid_inc_tax, Partial_MAX(ss_promo_sk) AS max$0)]
