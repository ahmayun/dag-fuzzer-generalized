...
autonode_2 = autonode_4.limit(76)
autonode_3 = autonode_5.order_by(col('cp_type_node_5'))
autonode_1 = autonode_2.join(autonode_3, col('cp_catalog_page_number_node_5') == col('wr_web_page_sk_node_4'))
sink = autonode_1.group_by(col('wr_return_ship_cost_node_4')).select(col('wr_returning_cdemo_sk_node_4').max.alias('wr_returning_cdemo_sk_node_4'))
print(sink.explain())
...
"""
{
  ...
  "result_details": {
    "opt": {
      ...
      "error_message": "An error occurred while calling o101051619.explain.
: java.lang.RuntimeException: Error while applying rule FlinkExpandConversionRule, args [rel#203675964:AbstractConverter.BATCH_PHYSICAL.hash[0]true.[8](input=RelSubset#203675962,convention=BATCH_PHYSICAL,FlinkRelDistributionTraitDef=hash[0]true,sort=[8]), rel#203675961:BatchPhysicalLocalHashAggregate.BATCH_PHYSICAL.any.[8](input=RelSubset#203675960,groupBy=cp_catalog_page_number,select=cp_catalog_page_number)]
...
LogicalProject(wr_returning_cdemo_sk_node_4=[$1])
+- LogicalAggregate(group=[{19}], EXPR$0=[MAX($8)])
   +- LogicalJoin(condition=[=($30, $11)], joinType=[inner])
      ...
      +- LogicalSort(sort0=[$8], dir0=[ASC])
         +- LogicalProject(cp_catalog_page_sk_node_5=[$0], cp_catalog_page_id_node_5=[$1], cp_start_date_sk_node_5=[$2], cp_end_date_sk_node_5=[$3], cp_department_node_5=[$4], cp_catalog_number_node_5=[$5], cp_catalog_page_number_node_5=[$6], cp_description_node_5=[$7], cp_type_node_5=[$8])
            +- LogicalTableScan(table=[[default_catalog, default_database, catalog_page]])

== Optimized Physical Plan ==
Calc(select=[EXPR$0 AS wr_returning_cdemo_sk_node_4])
+- HashAggregate(isMerge=[true], groupBy=[wr_return_ship_cost], select=[wr_return_ship_cost, Final_MAX(max$0) AS EXPR$0])
   +- Exchange(distribution=[hash[wr_return_ship_cost]])
      +- LocalHashAggregate(groupBy=[wr_return_ship_cost], select=[wr_return_ship_cost, Partial_MAX(wr_returning_cdemo_sk) AS max$0])
         +- NestedLoopJoin(joinType=[InnerJoin], where=[=(cp_catalog_page_number, wr_web_page_sk)], select=[wr_returned_date_sk, wr_returned_time_sk, wr_item_sk, wr_refunded_customer_sk, wr_refunded_cdemo_sk, wr_refunded_hdemo_sk, wr_refunded_addr_sk, wr_returning_customer_sk, wr_returning_cdemo_sk, wr_returning_hdemo_sk, wr_returning_addr_sk, wr_web_page_sk, wr_reason_sk, wr_order_number, wr_return_quantity, wr_return_amt, wr_return_tax, wr_return_amt_inc_tax, wr_fee, wr_return_ship_cost, wr_refunded_cash, wr_reversed_charge, wr_account_credit, wr_net_loss, cp_catalog_page_sk, cp_catalog_page_id, cp_start_date_sk, cp_end_date_sk, cp_department, cp_catalog_number, cp_catalog_page_number, cp_description, cp_type], build=[right])
            :- Limit(offset=[0], fetch=[76], global=[true])
            :  +- Exchange(distribution=[single])
            :     +- Limit(offset=[0], fetch=[76], global=[false])
            :        +- TableSourceScan(table=[[default_catalog, default_database, web_returns, limit=[76]]], fields=[wr_returned_date_sk, wr_returned_time_sk, wr_item_sk, wr_refunded_customer_sk, wr_refunded_cdemo_sk, wr_refunded_hdemo_sk, wr_refunded_addr_sk, wr_returning_customer_sk, wr_returning_cdemo_sk, wr_returning_hdemo_sk, wr_returning_addr_sk, wr_web_page_sk, wr_reason_sk, wr_order_number, wr_return_quantity, wr_return_amt, wr_return_tax, wr_return_amt_inc_tax, wr_fee, wr_return_ship_cost, wr_refunded_cash, wr_reversed_charge, wr_account_credit, wr_net_loss])
            +- Exchange(distribution=[broadcast])
               +- SortLimit(orderBy=[cp_type ASC], offset=[0], fetch=[1], global=[true])
                  +- Exchange(distribution=[single])
                     +- SortLimit(orderBy=[cp_type ASC], offset=[0], fetch=[1], global=[false])
