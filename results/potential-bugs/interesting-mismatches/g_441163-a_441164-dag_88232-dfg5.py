...
autonode_6 = autonode_9.order_by(col('sr_reversed_charge_node_9'))
autonode_5 = autonode_7.join(autonode_8, col('ca_address_sk_node_7') == col('cp_catalog_page_number_node_8'))
autonode_4 = autonode_5.join(autonode_6, col('sr_item_sk_node_9') == col('ca_address_sk_node_7'))
autonode_3 = autonode_4.group_by(col('ca_address_sk_node_7')).select(col('sr_return_time_sk_node_9').min.alias('sr_return_time_sk_node_9'))
autonode_2 = autonode_3.order_by(col('sr_return_time_sk_node_9'))
autonode_1 = autonode_2.order_by(col('sr_return_time_sk_node_9'))
sink = autonode_1.limit(55)
print(sink.explain())
...
LogicalSort(sort0=[$0], dir0=[ASC], fetch=[55])
+- LogicalSort(sort0=[$0], dir0=[ASC])
   +- LogicalProject(sr_return_time_sk_node_9=[$1])
      +- LogicalAggregate(group=[{0}], EXPR$0=[MIN($23)])
...
Calc(select=[EXPR$0 AS sr_return_time_sk_node_9])
+- SortLimit(orderBy=[EXPR$0 ASC], offset=[0], fetch=[55], global=[true])
   +- Exchange(distribution=[single])
      +- SortLimit(orderBy=[EXPR$0 ASC], offset=[0], fetch=[55], global=[false])
         +- Sort(orderBy=[EXPR$0 ASC])
            +- Exchange(distribution=[single])
               +- HashAggregate(isMerge=[true], groupBy=[ca_address_sk], select=[ca_address_sk, Final_MIN(min$0) AS EXPR$0])
                  +- Exchange(distribution=[hash[ca_address_sk]])
                     +- LocalHashAggregate(groupBy=[ca_address_sk], select=[ca_address_sk, Partial_MIN(sr_return_time_sk) AS min$0])
                        +- NestedLoopJoin(joinType=[InnerJoin], where=[=(sr_item_sk, ca_address_sk)], select=[ca_address_sk, ca_address_id, ca_street_number, ca_street_name, ca_street_type, ca_suite_number, ca_city, ca_county, ca_state, ca_zip, ca_country, ca_gmt_offset, ca_location_type, cp_catalog_page_sk, cp_catalog_page_id, cp_start_date_sk, cp_end_date_sk, cp_department, cp_catalog_number, cp_catalog_page_number, cp_description, cp_type, sr_returned_date_sk, sr_return_time_sk, sr_item_sk, sr_customer_sk, sr_cdemo_sk, sr_hdemo_sk, sr_addr_sk, sr_store_sk, sr_reason_sk, sr_ticket_number, sr_return_quantity, sr_return_amt, sr_return_tax, sr_return_amt_inc_tax, sr_fee, sr_return_ship_cost, sr_refunded_cash, sr_reversed_charge, sr_store_credit, sr_net_loss], build=[left])
                           :- Exchange(distribution=[broadcast])
                           :  +- HashJoin(joinType=[InnerJoin], where=[=(ca_address_sk, cp_catalog_page_number)], select=[ca_address_sk, ca_address_id, ca_street_number, ca_street_name, ca_street_type, ca_suite_number, ca_city, ca_county, ca_state, ca_zip, ca_country, ca_gmt_offset, ca_location_type, cp_catalog_page_sk, cp_catalog_page_id, cp_start_date_sk, cp_end_date_sk, cp_department, cp_catalog_number, cp_catalog_page_number, cp_description, cp_type], build=[right])
                           :     :- Exchange(distribution=[hash[ca_address_sk]])
                           ...
                           +- Sort(orderBy=[sr_reversed_charge ASC])
                              +- Exchange(distribution=[single])
...
",
    ...
    "unopt": {
      ...
      "error_message": "An error occurred while calling o240252332.explain.
: java.lang.RuntimeException: Error while applying rule FlinkExpandConversionRule, args [rel#485908197:AbstractConverter.BATCH_PHYSICAL.hash[0]true.[17](input=RelSubset#485908195,convention=BATCH_PHYSICAL,FlinkRelDistributionTraitDef=hash[0]true,sort=[17]), rel#485908194:BatchPhysicalLocalHashAggregate.BATCH_PHYSICAL.any.[17](input=RelSubset#485908193,groupBy=sr_item_sk,select=sr_item_sk, Partial_MIN(sr_return_time_sk) AS min$0)]
